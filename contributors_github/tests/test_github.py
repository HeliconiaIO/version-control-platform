...
class TestGithub(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.organization = cls.env["contributors.organization"].create(
            {
                "name": "oca",
                "key_ids": [
    ...
    def test_update_organization_and_logo(self):
        ...
        with (
            patch(
                "odoo.addons.contributors_github.models.contributors_organization.github3"
            ) as mock_github3,
            patch(
                "odoo.addons.contributors_github.models.contributors_organization.requests.get"
            ...
            self.organization.update_information()
            self.assertEqual(
                self.organization.short_description, "Odoo Community Association"
            )
    ...
    def test_update_organization_with_repository(self):
        with patch(
            ...
            mock_github3.login.return_value = mock_client
            self.organization.update_information()
            self.assertEqual(len(self.organization.repository_ids), 2)
            repo_names = {repo.name for repo in self.organization.repository_ids}
            self.assertSetEqual(repo_names, {"server-tools", "server-brand"})
            mock_github3.login.assert_called_once_with(
                token="ghp_exampletoken1234567890abcdef"
            )
            mock_client.organization.assert_called_once_with("oca")
        return self.organization.repository_ids.filtered(
            lambda r: r.name == "server-tools"
        )
    ...
    def test_update_repository(self):
        ...
        with patch(
            "odoo.addons.contributors_github.models.contributors_repository.github3.login"
        ) as mock_client:
        ...
        self.assertTrue(repository.pull_request_ids)
